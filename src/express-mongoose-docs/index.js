...
module.exports = function (app, mongoose) {
  ...
  const descriptions = JSON.parse(fs.readFileSync('src/apiDescription.json'));
  ...
  app.get('/api-docs', function (req, res) {
    try {
      ...
      routes = _.pairs(routes).map(route => {
        const search = searchApiDescription(route, descriptions);
        if (!_.isEmpty(search.description)) route[1][search.index].param = search.description.info.param;
        return route;
      });
...
function searchApiDescription(route, descriptions) {
  for (let i = 0; i < route[1].length; i++) {
    for (const description of descriptions) {
      if (description.path === route[1][i].path && description.method === route[1][i].method) {
        return {'index': i, description};
      }
    }
  }
  return {};
